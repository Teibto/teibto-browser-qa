...
| `bsk fill` ช่อง `<input type=date>` ตอบ `target element is not fillable` | fill ไม่รองรับ input ชนิดวันที่ | `click` ช่องนั้นแล้ว `press` ตัวเลขทีละตัวตามลำดับที่ locale ของ browser แสดง (en-US: MMDDYYYY) แล้วอ่าน `.value` ยืนยันเป็น `yyyy-mm-dd` (2026-09-24 · `verified`) |
...
| `bsk fill` ช่องพิมพ์ของ Slack (`.ql-editor` / `[data-qa="texty_input"]`) ตอบ `the fill target changed during the action` หรือ `fill could not verify the expected value` แต่ข้อความเข้าจริง | composer เป็น Quill contenteditable — คลาส `ql-blank`/node เปลี่ยนตอน focus ทำให้ขั้นตรวจค่าไม่ผ่าน; `observe` ไล่ tree ฝั่ง sidebar ไม่ถึง main panel จึงไม่เห็น `textbox "Message to <channel>"` | เอา ref จาก `snapshot` (ไม่ใช่ `observe`) หรือใช้ selector `[data-qa="texty_input"]` แล้ว `fill --no-clear` · **อ่านค่าจริงด้วย `evaluate` ก่อนตัดสิน ห้าม retry รัว** · ส่งด้วย `press Enter --selector '[data-qa="texty_input"]'` (2026-09-24 · `verified`) |
