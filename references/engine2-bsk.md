...
| บน Windows Git Bash (MINGW64) `bsk press … /` ตอบ `error: invalid command parameters` / `details: press: unknown key "C:/Program Files/Git/"` (exit 1) แทนที่จะกดปุ่ม `/` — และไม่เห็นอาการนี้ถ้ารันกับ session ปลอม | MSYS ของ Git Bash แปลงอาร์กิวเมนต์ที่เป็น `/` ให้เป็น path ของ Git ก่อนส่งให้ `bsk.exe`; อาการโผล่หลัง bsk resolve session สำเร็จ (`python -c "import sys;print(sys.argv[1:])" /` → `['C:/Program Files/Git/']` ยืนยันกลไกได้โดยไม่ต้องมี session) | ตั้ง `MSYS_NO_PATHCONV=1` เฉพาะคำสั่งนั้น: `MSYS_NO_PATHCONV=1 bsk press … --tab-id N /` → `press ok … key=/` · ใช้ทุกครั้งที่ส่งค่าที่ขึ้นต้นด้วย `/` ผ่าน Git Bash (bsk 0.3.1 · Windows Git Bash MINGW64 · 2026-09-24 · `verified`) |
