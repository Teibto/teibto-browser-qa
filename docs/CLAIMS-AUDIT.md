...
| Two `bsk` daemons with different `BSK_HOME` run side by side: `daemon start --port <P>` listens on that port and each home gets its own named pipe and `daemon.lock` | measured, version-pinned | bsk 0.3.1 + Chrome 153, Windows, 2026-09-23: default home on 52800 (`bsk-daemon-58b761a6f49c81db`) and `~/.bsk-foodstar` on 52810 (`bsk-daemon-19e46a8f15a66262`), both `LISTENING` in `netstat -ano`, `bsk status` answered per home |
| The BrowserSkill extension connects to one port per Chrome profile, set by **Local port** → **Save port**; the switch is immediate | measured, version-pinned | same machine: daemon logs show `browser disconnected id=3e08257e` on 52800 at 14:06:49.448 and `browser connected id=3e08257e` on 52810 at 14:06:49.451; an earlier reported save left the extension storage unchanged, so the result must be checked, not assumed |
| Several projects on one daemon and one Chrome profile made their sessions overlap and dropped the NetSuite login | inferred | user report 2026-09-23 while three projects ran at once; consistent with the §4 row on one NetSuite user in two browsers; not A/B-tested |
| `scripts/bsk-account.py` gives each account its own home, port (never 52800, never another account's) and session registry; reuses a running daemon; refuses to move an account's port or reuse a daemon on the wrong port; stores no secret | verified | `tests/test_bsk_account.py` (12 cases against the per-home fake daemon in `tests/fixtures/fake_bsk.py`); live 2026-09-23: `ensure foodstar --port 52810 --bsk-home ~/.bsk-foodstar` adopted the running daemon (pid 26972, browser `3e08257e`, `started: false`) and left the 52800 daemon untouched |
